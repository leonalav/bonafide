...
import { reduce, makeInitialState, type IdeState, type IdeAction, type FileNode } from "../src/ide/store";
...
expect(state.tabs.length, 0, "CLOSE_TAB after SET_CONTENT actually closes the tab");
...
state = dispatch(state, { type: "MARK_DIRTY", tabId: untitledTab?.id, dirty: true });
...
state = dispatch(state, { type: "CLOSE_TAB", tabId: untitledTab?.id, force: true });
expect(state.modal, null, "CLOSE_TAB with force=true skips the modal");
expect(state.tabs.length, 0, "CLOSE_TAB with force=true actually closes the dirty tab");
