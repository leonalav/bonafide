...
type ToastArgs = { message: string; tone: "info" | "success" | "error"; ttlMs: number };
...
export function buildPaletteCommands(
  ...
  async function openWorkspace(folderPath: string): Promise<void> {
    try {
      ...
      pushToast(dispatch, { message: "Failed to open folder", tone: "error", ttlMs: 2400 });
    }
  }
  ...
  return [
    ...
    // ── Run ─────────────────────────────────────────────────────────────────
    {
      ...
      icon: "shield-check",
      when: () =>
        !!activeTab && !!activeTab.fileId.endsWith(".py"),
      run: () => {
        if (!activeTab) return;
        window.dispatchEvent(
