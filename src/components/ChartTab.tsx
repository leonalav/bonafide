    ...
    <div className="flex min-h-0 flex-1 flex-col bg-surface">
      ...
      <div className="shrink-0 border-b border-outline-variant px-6 py-4">
        <div className="flex items-baseline gap-3">
          <h1 className="font-sans text-[18px] font-medium text-on-surface">
            {m.label}
          </h1>
        ...
        <div className="mx-auto max-w-[960px]">
          <div className="rounded-lg border border-outline-variant bg-surface-container-low p-4">
            <Chart
              ...
              yTicks={6}
              xTicks={8}
              ariaLabel={`${m.label} for ${run.name}`}
            />
          </div>
          ...
          <div className="mt-4 grid grid-cols-2 gap-3 sm:grid-cols-4">
            ...
            <Stat
              label="Best"
              value={fmtValue(
                m.lowerIsBetter ? Math.min(...m.series) : Math.max(...m.series),
              )}
            />
            <Stat
              label="Min → Max"
              value={`${fmtValue(Math.min(...m.series))} – ${fmtValue(Math.max(...m.series))}`}
            />
            <Stat
              label="Δ vs prior"
              value={
                <Chip
                  tone={
                    m.delta === 0 ? "neutral" : better ? "positive" : "negative"
                  }
                >
                  {m.delta === 0
                    ? "—"
...
function Stat({ label, value }: { label: string value: React.ReactNode }) {
  return (
    <div className="rounded-lg border border-outline-variant bg-surface-container-low p-3">
      ...
      <div className="mt-1 font-sans text-[15px] tabular-nums text-on-surface">
        {value}
      </div>
    </div>
  )
