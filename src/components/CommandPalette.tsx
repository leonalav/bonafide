...
export default function CommandPalette() {
  ...
  // Local query state mirrors store for immediate typing feedback.
  // Synced to store on change so hooks can observe it.
  const [query, setQuery] = useState(state.palette.query);
  const [selectedIndex, setSelectedIndex] = useState(state.palette.selected);

  const inputRef = useRef<HTMLInputElement>(null);
  const listRef = useRef<HTMLDivElement>(null);

  // Build and filter commands.
  const allCommands = useMemo(() => buildPaletteCommands(store), [store]);
  const filteredCommands = useMemo(
    () => filterCommands(allCommands, query, state),
    [allCommands, query, state],
  );
  const grouped = useMemo(() => groupCommands(filteredCommands), [filteredCommands]);
  ...
  return createPortal(
    ...
    <div
      ...
      className="fixed inset-0 z-[9995] flex items-start justify-center bg-black/40 backdrop-blur-md pt-[12vh]"
      onKeyDown={onKeyDown}
      onClick={(e) => {
    ...
    >
      ...
      <div
        style={{ width: 640 }}
        className="w-[640px] max-h-[70vh] flex flex-col rounded border border-outline/20 bg-surface-container-lowest/80 backdrop-blur-xl shadow-2xl overflow-hidden"
      >
        {/* Search input */}
        <div className="flex items-center gap-3 border-b border-outline/20 px-4 py-3">
          ...
          <input
            ...
            className="h-10 flex-1 bg-transparent font-body text-[14px] text-on-surface placeholder:text-outline focus:outline-none"
          />
          {query && (
            <button
        ...
        <div className="flex items-center gap-4 border-t border-outline-variant/40 px-4 py-2 font-sans text-[12px] text-outline">
          <span>
            <kbd className="rounded bg-surface-container px-1.5 py-0.5 font-mono text-[11px]">↑↓</kbd>{" "}
            navigate
          </span>
          <span>
            <kbd className="rounded bg-surface-container px-1.5 py-0.5 font-mono text-[11px]">↵</kbd>{" "}
          ...
          <span>
            <kbd className="rounded bg-surface-container px-1.5 py-0.5 font-mono text-[11px]">Esc</kbd>{" "}
            close
          </span>
        </div>
      </div>
