...
function flipPosition(target: MenuBounds, win: Window): MenuBounds {
  ...
  const x = target.x + w > innerW - SAFE_GAP
    ? Math.max(SAFE_GAP, innerW - w - SAFE_GAP)
    : target.x;

  const y = target.y + h > innerH - SAFE_GAP
...
function MenuItemRenderer({
  ...
  return (
    ...
    >
      ...
      >
        {item.icon && (
          <Icon name={item.icon} size={14} className="text-on-surface-variant" />
        )}
        <span className="flex-1 truncate text-left">{item.label}</span>
...
function SubmenuPortal({ items, onClose }: { items: MenuItem[]; onClose: () => void }) {
  return createPortal(
    <div
