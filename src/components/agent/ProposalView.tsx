...
function Region({ label, right, children }: { label: string; right?: React.ReactNode; children: React.ReactNode }) {
  return (
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <Region
        label="Actions"
        right={
          <button onClick={() => setTraceOpen((v) => !v)} className="font-sans text-[11px] text-outline hover:text-on-surface">
      ...
      >
        <div className="overflow-hidden rounded border border-outline-variant bg-surface">
          {shown.map((s, i) => {
            ...
            return (
              <div key={i} className="border-b border-outline-variant/50 last:border-b-0">
                <button
                  onClick={() => setExpanded((e) => ({ ...e, [i]: !e[i] }))}
                  className="flex w-full items-center gap-2 px-2 py-1.5 text-left hover:bg-surface-container/50"
                >
                  <Icon name={open ? "chevron-down" : "chevron-right"} size={12} className="text-outline" />
                  ...
                  <span className="shrink-0 font-sans text-[11px] text-outline">{s.time}</span>
                </button>
                {open && s.result && (
      ...
      <Region label="Hypothesis">
        <div className="flex flex-col gap-3 rounded border border-outline-variant bg-surface-container-low p-3">
          <div className="flex gap-2">
            <Icon name="flag" size={15} className="mt-0.5 shrink-0 text-tertiary" />
            <p className="font-body text-[13px] leading-[19px] text-on-surface">
              <span className="font-medium text-tertiary">{data.hypothesis.verdict}:</span> {data.hypothesis.statement}
            </p>
          </div>
          <div>
            ...
            <div className="flex flex-col gap-1">
              {data.hypothesis.evidence.map((e, i) => (
                <button key={i} className="group flex items-center gap-1.5 text-left font-sans text-[12px] text-on-surface-variant hover:text-primary">
                  <span className="text-outline group-hover:text-primary">•</span>
                  <span className="flex-1">{e.text}</span>
                  {e.jump && <Icon name="corner-down-right" size={12} className="text-outline group-hover:text-primary" />}
                </button>
              ))}
            </div>
          </div>
          <div className="flex items-center justify-between border-t border-outline-variant/50 pt-2 font-sans text-[12px]">
            <span className="text-on-surface-variant">
              Confidence <span className={`font-medium ${CONF_TONE[data.hypothesis.confidence]}`}>{data.hypothesis.confidence}</span>
            </span>
            {data.hypothesis.ruledOut && <span className="text-outline">Ruled out: {data.hypothesis.ruledOut}</span>}
          </div>
        </div>
      </Region>
        ...
        <div className="overflow-hidden rounded border border-outline-variant">
          <div className="font-sans text-[13px] leading-[22px]">
            {data.patch.lines.map((l, i) => (
              ...
              >
                <span className={`w-6 shrink-0 text-center ${l.sign === "+" ? "text-primary" : l.sign === "-" ? "text-error" : "text-outline"}`}>
                  {l.sign === " " ? "" : l.sign}
                </span>
          ...
          >
            <div className="mb-1 flex items-center gap-2 font-sans text-[13px]">
              {data.verification.status === "running" ? (
                <Icon name="refresh" size={14} className="animate-sync-spin text-tertiary" />
              ) : (
                <Icon name={data.verification.status === "success" ? "check" : "x"} size={14} className={data.verification.status === "success" ? "text-primary" : "text-error"} />
              )}
              <span className="font-medium text-on-surface">{data.verification.lines[0]}</span>
            </div>
            {data.verification.lines.slice(1).map((l, i) => (
              <p key={i} className="pl-6 font-sans text-[12px] text-on-surface-variant">{l}</p>
            ))}
            {data.verification.upgrade && (
              <p className="pl-6 font-sans text-[12px] text-primary">Confidence upgraded: {data.verification.upgrade}</p>
            )}
          </div>
        </Region>
      )}
      ...
      <div className="flex items-center justify-end gap-1.5 border-t border-outline-variant pt-4">
        <Button variant="ghost" size="sm" className="whitespace-nowrap !text-error hover:bg-error/10">Reject</Button>
        <Button variant="ghost" size="sm" className="whitespace-nowrap hover:!text-on-surface">Request revision</Button>
        <Button size="sm" className="whitespace-nowrap">Approve &amp; apply</Button>
      </div>
    </div>
  );
}
