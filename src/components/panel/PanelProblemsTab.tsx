...
const SEVERITY_ICON: Record<DiagnosticEntry["severity"], { icon: string; color: string }> = {
  ...
  return (
    <div
      ...
      className={`group flex cursor-pointer items-start gap-2 border-b border-outline-variant/40 px-3 py-1.5 font-mono text-[12px] transition-colors hover:bg-surface-container-high ${
        diag.severity === "error" ? "bg-red-950/20" : diag.severity === "warning" ? "bg-yellow-950/10" : ""
      }`}
    >
      ...
      {diag.code && (
        <span className="shrink-0 text-outline">{diag.code}</span>
      )}
      {diag.fileLabel && (
      ...
      <span className="shrink-0 text-[11px] text-outline-variant">{diag.source}</span>
    </div>
  );
}
    ...
    return (
      <div className="flex h-full items-center justify-center font-sans text-[13px] text-outline">
        <div className="text-center">
          <Icon name="check" size={24} className="mx-auto mb-2 text-green-500" />
          <p>No Problems</p>
          <p className="mt-1 text-[12px]">No errors, warnings, or hints detected.</p>
        </div>
      </div>
    );
  }
