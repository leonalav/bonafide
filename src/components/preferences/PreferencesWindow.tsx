...
export function PreferencesWindow({
  ...
  return (
    <div className="absolute inset-0 z-[60] flex items-center justify-center bg-black/40 p-6 backdrop-blur-[2px]" onMouseDown={onClose}>
      <div
      ...
      >
        ...
        <div className="flex h-8 shrink-0 items-center border-b border-outline-variant bg-surface-container-lowest px-3">
          ...
          <div className="flex flex-1 items-center justify-center gap-2">
            ...
            <span className="font-sans text-[13px] font-semibold text-on-surface">Bonafide — Preferences</span>
          </div>
          <div className="flex items-center gap-3 text-outline">
            ...
            <button onClick={onClose} aria-label="Close preferences" className="hover:text-on-surface">
              <Icon name="x" size={14} />
            </button>
          </div>
        </div>
        ...
        <div className="flex min-h-0 flex-1">
          ...
          <nav className="flex w-60 shrink-0 flex-col border-r border-outline-variant bg-surface-container-low pt-4">
            <div className="label-caps px-4 pb-2 text-on-surface-variant">Preferences</div>
            {NAV.map((item) => {
              ...
              return (
                ...
                >
                  ...
                  <Icon name={item.icon} size={16} className={active ? "text-on-surface" : "text-on-surface-variant"} />
                  {item.label}
                </button>
              );
            })}
            ...
            <div className="mx-3 border-t border-outline-variant py-3">
              <div className="flex items-center gap-2 font-sans text-[12px] text-on-surface-variant">
                <span className="h-1.5 w-1.5 rounded-full bg-primary" /> Connected to W&amp;B
              </div>
              <div className="mt-1 flex items-center gap-1.5 font-sans text-[12px] text-on-surface-variant">
                <Icon name="git-branch" size={11} className="text-outline" /> main · a3f9c12
              </div>
