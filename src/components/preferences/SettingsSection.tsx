...
export const SettingsSection = forwardRef<HTMLDivElement, {
  highlightTrackers?: boolean
}>(function SettingsSection({ highlightTrackers }, trackersRef) {
    ...
    <div className="mx-auto flex max-w-[720px] flex-col gap-8">
      ...
      <Card>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Field label="Font size">
            ...
            <span
              className="ml-2 font-sans text-[16px]"
      ...
      <Card>
        ...
        <div className="flex flex-col gap-2">
          ...
          <Field label="Virtual env">
            <span className="flex items-center gap-1 font-sans text-[13px] text-on-surface">
              bonafide-train (.venv){" "}
          ...
          <div className="flex items-center justify-between gap-4">
            ...
            <span className="truncate font-sans text-[12px] text-on-surface-variant">
              torch 2.2.1 · transformers 4.36 · wandb 0.16
            </span>
          </div>
        </div>
        <div className="my-3 h-px bg-outline-variant/60" />
        <div className="label-caps mb-1 text-outline">
      ...
      <div
        ...
        className={`rounded-lg transition-shadow ${
          highlightTrackers ? "ring-2 ring-primary" : ""
        }`}
      >
        <Card>
          ...
          <Field label="Default project">
            <input
              defaultValue="bonafide-train"
              className="h-7 w-56 rounded border border-outline-variant bg-surface px-2 font-sans text-[13px] text-on-surface focus:border-primary focus:outline-none"
            />
          </Field>
          <div className="mt-3">
          ...
          <div className="mt-3">
            <Field label="Delta chip metric">
              <Select
                value={deltaMetric}
                onChange={setDeltaMetric}
                align="right"
                className="w-40"
                options={["val_loss", "loss", "acc", "lr"]}
              />
            </Field>
          </div>
        </Card>
      ...
      <Card>
        ...
        <div className="label-caps mb-1 mt-3 text-outline">
          When a run fails
        </div>
        <Checkbox label="Toast in app" defaultChecked />
        <Checkbox label="Native OS notification" defaultChecked />
        ...
        <div className="mt-3 flex items-center gap-2">
          ...
          <input
            type="time"
            defaultValue="22:00"
            className="h-7 rounded border border-outline-variant bg-surface px-2 font-sans text-[12px] text-on-surface focus:border-primary focus:outline-none"
          />
          <span className="font-body text-[13px] text-on-surface-variant">
            to
          </span>
      ...
      <Card>
        ...
        <Checkbox
          ...
          description="Helps improve Bonafide. No code or run data is ever sent."
        />
        <div className="h-2" />
        <Checkbox
          label="Send crash reports"
          description="Includes stack traces only, no file contents."
        />
        <div className="my-3 h-px bg-outline-variant/60" />
        ...
        <Field label="Cache location">
          <span className="font-sans text-[12px] text-on-surface-variant">
            ~/Library/Application Support/Bonafide
          </span>
        </Field>
        ...
        <Field label="Cache size">
          <span
            aria-live="polite"
            className="font-sans text-[13px] tabular-nums text-on-surface"
          >
            247 MB
          </span>
          <Button variant="secondary" size="sm">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            <Button variant="ghost" size="sm" className="hover:!text-error">
              Delete account
            </Button>
          </div>
        </div>
      ...
      <Card>
        <div className="flex items-center justify-between">
          ...
          >
            <Icon name={advanced ? "chevron-up" : "chevron-down"} size={13} />{" "}
            {advanced ? "Collapse" : "Expand"}
          </button>
        </div>
        {advanced && (
          <div className="mt-3 flex flex-col gap-2">
            <Field label="IPC port">
              <span className="font-sans text-[13px] text-on-surface">
                7654
              </span>
            </Field>
            <Field label="Python shim socket">
              <span className="font-sans text-[12px] text-on-surface-variant">
                ~/.bonafide/shim.sock
              </span>
            </Field>
            <Field label="Local DB">
              <span className="font-sans text-[12px] text-on-surface-variant">
                …/Bonafide/cache.db
              </span>
            </Field>
            ...
            <Checkbox
              ...
              description="Writes to ~/.bonafide/logs/"
            />
