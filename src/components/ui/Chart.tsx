...
export function Chart({
  ...
  const yTickVals = Array.from({ length: yTicks + 1 }, (_, k) => domMin + (span * k) / yTicks);
  const xTickIdx = Array.from({ length: xTicks }, (_, k) => Math.round((k / (xTicks - 1)) * (n - 1)));
  ...
  return (
    <div ref={wrapRef} className="relative w-full overflow-hidden" style={{ height }}>
      {ready && (
        <svg width={w} height={height} role="img" aria-label={ariaLabel ?? "metric chart"} className="block">
          {/* Y grid + ticks */}
          {yTickVals.map((v, i) => {
            const y = yAt(v);
            return (
              <g key={`y${i}`}>
                <line x1={mL} y1={y} x2={w - mR} y2={y} stroke="var(--color-outline-variant)" strokeWidth={1} opacity={0.35} />
                <text x={mL - 6} y={y + 3} textAnchor="end" className="fill-[var(--color-outline)]" style={{ fontSize: 10 }}>
                  {fmtValue(v)}
                </text>
              </g>
            );
          ...
          {xTickIdx.map((idx, i) => {
            const x = xAt(idx);
            return (
              <text key={`x${i}`} x={x} y={height - 6} textAnchor={i === 0 ? "start" : i === xTickIdx.length - 1 ? "end" : "middle"} className="fill-[var(--color-outline)]" style={{ fontSize: 10 }}>
                {fmtStep(stepAt(idx))}
              </text>
            );
          })}
          ...
          {/* Axis baseline */}
          <line x1={mL} y1={mT + plotH} x2={w - mR} y2={mT + plotH} stroke="var(--color-outline-variant)" strokeWidth={1} />
          ...
          {hover != null && (
            <g>
              <line x1={xAt(hover)} y1={mT} x2={xAt(hover)} y2={mT + plotH} stroke={stroke} strokeWidth={1} opacity={0.5} strokeDasharray="3 3" />
              <circle cx={xAt(hover)} cy={yAt(data[hover])} r={3.5} fill="var(--color-surface)" stroke={stroke} strokeWidth={2} />
            </g>
          )}
      ...
      {ready && hover != null && (
        ...
        >
          <div className="font-sans text-[11px] text-outline">step {fmtStep(stepAt(hover))}</div>
          <div className="font-sans text-[13px] font-medium tabular-nums text-on-surface">{fmtValue(data[hover])}</div>
        </div>
      )}
    </div>
  );
