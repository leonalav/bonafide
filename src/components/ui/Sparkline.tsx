...
export function Sparkline({
  ...
  return (
    <svg
      ...
      role="img"
      aria-label={ariaLabel ?? `trend, final value ${data[data.length - 1].toFixed(2)}`}
      className={responsive ? "block max-w-full" : ""}
    >
      <title>{`${ariaLabel ? ariaLabel + " — " : ""}min ${min.toFixed(2)} · max ${max.toFixed(2)} · last ${data[data.length - 1].toFixed(2)}`}</title>
...
export function blockSpark(data: number[], n = 10): string {
  ...
  const span = max - min || 1;
  return s.map((v) => BLOCKS[Math.min(7, Math.floor(((v - min) / span) * 7.999))]).join("");
}
