  ...
  return (
    <label className="flex cursor-pointer items-start gap-2 py-0.5">
      ...
      <span className="min-w-0">
        <span className="block font-body text-[13px] text-on-surface">
          {label}
        </span>
        {description && (
          <span className="mt-0.5 block font-body text-[12px] leading-4 text-on-surface-variant">
            {description}
          </span>
        )}
      </span>
      ...
      <div className="inline-flex overflow-hidden rounded border border-outline-variant">
        {options.map((o, i) => (
          <button
            ...
            className={`h-7 px-3 font-sans text-[12px] transition-colors ${
    ...
    <div className="flex flex-wrap items-center gap-4">
      {options.map((o) => (
        <button
          key={o.value}
          onClick={() => onChange(o.value)}
          className="flex items-center gap-1.5 font-body text-[13px] text-on-surface-variant hover:text-on-surface"
        >
          <span
            className={`flex h-4 w-4 items-center justify-center rounded-full border ${
              value === o.value ? "border-primary" : "border-outline"
            }`}
          >
            {value === o.value && (
...
export function Card({
  ...
  return (
    <div
      className={`rounded-lg border border-outline-variant bg-surface-container p-4 ${className}`}
    >
      {children}
    </div>
  )
