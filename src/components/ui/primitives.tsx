...
const VARIANT: Record<Variant, string> = {
  ...
  secondary: "bg-transparent border border-outline-variant text-secondary hover:bg-surface-container hover:border-outline",
  ghost: "bg-transparent text-outline hover:text-primary hover:bg-surface-container/60",
...
export function StatusDot({ token, pulse, size = 8 }: { token: DotToken; pulse?: boolean; size?: number }) {
  return (
    <span
      className={`inline-block shrink-0 rounded-full ${DOT_BG[token]} ${pulse ? "animate-dot-pulse" : ""}`}
...
export function Chip({ tone = "neutral", children }: { tone?: ChipTone; children: ReactNode }) {
  return (
    <span className={`inline-flex items-center gap-1 rounded px-1.5 py-0.5 font-sans text-[12px] font-medium tabular-nums ${CHIP[tone]}`}>
      {children}
    </span>
  );
}
...
export function SectionLabel({ children, className = "" }: { children: ReactNode; className?: string }) {
  return <div className={`label-caps text-on-surface-variant ${className}`}>{children}</div>;
}
