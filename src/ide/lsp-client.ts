...
export async function getOrCreateClient(
  ...
  try {
    ...
    const client = new LanguageServerClient({
      rootUri,
      workspaceFolders: [{ uri: rootUri, name: workspaceRoot.split(/[\\/]/).pop() ?? "workspace" }],
      transport,
    });
    ...
    console.error(`[lsp] Failed to create ${serverKind} client for ${workspaceRoot}:`, err);
    return null;
  }
}
...
export function closeClient(serverKind: ServerKind, workspaceRoot: string): void {
  const rootUri = pathToUri(workspaceRoot);
  const key = cacheKey(serverKind, rootUri);
 ...
 */
export function documentUriFor(workspaceRoot: string, filePath: string): string {
