...
} | { kind: "closeDirty" tabId: string fileId: string name: string } | null
...
export type IdeAction = {
  ...
  nodeId: string
} | { type: "COLLAPSE_ALL" } | { type: "EXPAND_ALL" } | {
  type: "OPEN_FILE"
  fileId: string
} | { type: "OPEN_UNTITLED" } | {
  type: "OPEN_VIRTUAL_TAB"
  virtualId: "chart" | "experiments"
  name: string
...
{ type: "OPEN_PALETTE" } | { type: "CLOSE_PALETTE" } | {
  ...
  switch (action.type) {
    ...
    case "RENAME": {
      ...
      const tabs = state.tabs.map((t) =>
        t.fileId === nodeId
          ? { ...t, name: renamedNode.name, fileId: renamedNode.id }
          : t,
      )
    ...
    case "DELETE": {
      ...
      const deletedIds = new Set(
        removed.filter((n) => n.kind === "file").map((n) => n.id),
      )
    ...
    case "OPEN_MERGE_REVIEW": {
      ...
      if (existingTab) {
        ...
        const updatedReviews = state.mergeReviews.map((r) =>
          r.absPath === absPath
            ? { ...r, originalText, proposedText, source }
            : r,
        )
    ...
    case "SET_PANEL_HEIGHT": {
      return {
        ...
        panel: {
          ...state.panel,
          height: Math.max(100, Math.min(600, action.height)),
        },
      }
    }
...
export class IdeStore {
  ...
  dispatch(action: IdeAction): void {
    ...
    perfLog("IdeStore", "dispatch:reducer_done", {
